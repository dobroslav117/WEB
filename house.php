...
<section class="bg-white -mt-12 py-16">
    <div class="flex flex-wrap items-center justify-center min-h-screen gap-[50px] lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1">
      ...
      $servername = "127.0.0.1:3306"; // або IP-адреса сервера бази даних
    $username = "root";
    $password = "";
    $database = "lab3";
...
$limit = isset($_GET['houses']) ? intval($_GET['houses']) : 3;

$sql = "SELECT * FROM Houses LIMIT $limit";
...
?>
        <?php while($row = $result->fetch_assoc()): ?>
            ...
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
              ...
              <div class="p-4">
                <p class="uppercase tracking-wide text-sm font-bold text-gray-700"><?php echo $row['name']; ?></p>
                <?php
                    if ($discount !== false) {
                        $discountedPrice = $row['price'] - ($row['price'] * $discount / 100);
                        echo '<p style="font-size: 18px; color: red; text-decoration: line-through;">$' . $row['price'] . '</p>';
                        echo '<p style="font-size: 24px; color: red;">$' . $discountedPrice . '</p>';
                    } else {
                        // Якщо знижки немає, виводимо просто поточну ціну
                        echo '<p class="text-3xl text-gray-900">$' . $row['price'] . '</p>';
                    }
                ?>
                <!-- <p class="text-3xl text-gray-900">$<?php echo $house->price; ?></p> -->
                <p class="text-gray-700"><?php echo $row['houseAdress']; ?></p>
              </div>
              <div class="flex p-4 border-t border-gray-300 text-gray-700">
                <div class="flex-1 inline-flex items-center">
                  ...
                  <p><span class="text-gray-900 font-bold"><?php echo $row['bedrooms']; ?></span> Bedrooms</p>
                </div>
                <div class="flex-1 inline-flex items-center">
                  <svg class="h-6 w-6 text-gray-600 fill-current mr-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                  ...
                  <p><span class="text-gray-900 font-bold"><?php echo $row['bathrooms']; ?></span> Bathrooms</p>
                </div>
              </div>
              <div class="px-4 pt-3 pb-4 border-t border-gray-300 bg-gray-100">
                <div class="text-xs uppercase font-bold text-gray-600 tracking-wide">Realtor</div>
                <div class="flex items-center pt-2">
                  <div class="bg-cover bg-center w-10 h-10 rounded-full mr-3" style="background-image: url(<?php echo $row['ImgRealtor']; ?>)">
                  </div>
                  <div>
                    <p class="font-bold text-gray-900"><?php echo $row['realtor']; ?></p>
                    <p class="text-sm text-gray-700"><?php echo $row['phone']; ?></p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class=" w-full flex ">
      ...
      <form method="get" class="m-auto" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <input type="hidden" name="houses" value="<?php echo isset($_GET['houses']) ? intval($_GET['houses']) + 3 : 6; ?>">
        <?php
          ...
          {
            echo '<button type="submit" class="m-auto bg-green-500 px-8 py-5 rounded-lg text-white hover:bg-green-900 duration-300">Show more</button>';
          } ?>
      </form>
        <?php
            // Перевірка, чи передано параметр houses у GET-запиті
            if (isset($_GET['houses'])) {
                // Отримання значення параметра houses
                $houses = intval($_GET['houses']); // intval() перетворює значення на ціле число
                // Виведення нового значення houses
                //echo "Кількість будинків: " . $houses;
            }
        ?>
    </div>
  </section>